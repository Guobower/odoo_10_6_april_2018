...
class FundsFlow(models.Model): 
	...
	total_jv = fields.Float(string = "Total JV", compute = "get_totals", store = True,digits=(19,0))
	total_receipts = fields.Float(string = "Total Receipts", compute = "get_totals", store = True,digits=(19,0))
	total_payments = fields.Float(string = "Total Payments", compute = "get_totals", store = True,digits=(19,0))
...
class FundsFLowTree(models.Model): 
	...
	entry_level = fields.Float(string = "Entry Level",digits=(19,0))
	clearance_level = fields.Float(string = "Clearance Level",digits=(19,0))
	virtual_balance = fields.Float(string = "Virtual Balance",digits=(19,0))
	actual_balance = fields.Float(string = "Actual Balance",digits=(19,0))
	...
	def create(self, vals):
		...
		if new_record.type_transaction == "br":
			...
			create_debit = journal_entries_lines.create({
				...
				'name':"Received" + " "+ str(new_record.amount) + "  " + "From" + "  "+ str(new_record.banks_pakistan.name if new_record.banks_pakistan else " ")+str(new_record.bank_code if new_record.banks_pakistan else " ") + " " +str(new_record.party.name)+  " in " + str(new_record.bank.code if new_record.bank else " ")+ " " +str(new_record.bank.name if new_record.bank else " ")+  " "  + str(new_record.remarks if new_record.remarks else ""), 
		...
		if new_record.type_transaction == "bp":
			...
			create_debit = journal_entries_lines.create({
				'account_id':party_ledger.id,
				'partner_id':new_record.party.id,
				'name':"Paid" + " "+ str(new_record.amount) + "  " + "To" + "  "+str(new_record.party.name) + " in " + str(new_record.banks_pakistan.name if new_record.banks_pakistan else " ") + " " + str(new_record.bank_code if new_record.banks_pakistan else " ") + " " +"From "+ str(new_record.bank.code if new_record.bank else " ")+ " " +str(new_record.bank.name if new_record.bank else " ")+ " "+ str(new_record.remarks if new_record.remarks else ""), 
				'debit':new_record.amount,
				'move_id':create_journal_entry.id
				})
			create_credit = journal_entries_lines.create({
		...
		if new_record.type_transaction == "jv":
			...
			create_debit = journal_entries_lines.create({
				...
				'name':"Payment from" + " "+ str(new_record.customer.name) + "  " + str(new_record.banks_pakistan.name if new_record.banks_pakistan else " ")+ str(new_record.bank_code if new_record.banks_pakistan else " ")+" "+"To" + "  "+str(new_record.supplier.name) + " in " + str(new_record.supplier_bank.name if new_record.supplier_bank else " ") + " " + str(new_record.supplier_bank_code if new_record.supplier_bank else " ") + " " + str(new_record.remarks if new_record.remarks else ""), 
				'debit':new_record.amount,
				'move_id':create_journal_entry.id
				})
			create_credit = journal_entries_lines.create({
	...
	def write(self, vals):
		...
		if self.type_transaction == "br":
			...
			create_debit = journal_entries_lines.create({
				...
				'name':"Received" + " "+ str(self.amount) + "  " + "From" + "  "+ str(self.banks_pakistan.name if self.banks_pakistan else " ")+str(self.bank_code if self.banks_pakistan else " ") + " " +str(self.party.name)+  " in " + str(self.bank.code if self.bank else " ")+ " " +str(self.bank.name if self.bank else " ")+  " "  + str(self.remarks if self.remarks else ""), 
				'debit':self.amount,
			...
			# 		y.name = "Received" + " "+ str(self.amount) + "  " + "From" + "  "+str(self.banks_pakistan.name if self.banks_pakistan else " ")+str(self.bank_code if self.banks_pakistan else " ")+" "+str(self.party.name)+ " in " + str(self.bank.code if self.bank else " ") +str(self.bank.name if self.bank else " ")+  " "  + str(self.remarks if self.remarks else "")
			# 		y.debit = self.amount
			# 	else:
			# 		y.partner_id = self.party.id
			# 		y.name = "Received" + " "+ str(self.amount) + "  " + "From" + "  "+str(self.banks_pakistan.name if self.banks_pakistan else " ")+str(self.bank_code if self.banks_pakistan else " ")+" "+str(self.party.name)+ " in " + str(self.bank.code if self.bank else " ") +str(self.bank.name if self.bank else " ")+  " "  + str(self.remarks if self.remarks else "")
			...
			create_debit = journal_entries_lines.create({
				'account_id':party_ledger.id,
				'partner_id':self.party.id,
				'name':"Paid" + " "+ str(self.amount) + "  " + "To" + "  "+str(self.party.name) + " in " + str(self.banks_pakistan.name if self.banks_pakistan else " ") + " " + str(self.bank_code if self.banks_pakistan else " ") + " " +"From "+ str(self.bank.code if self.bank else " ")+ " " +str(self.bank.name if self.bank else " ")+ " "+ str(self.remarks if self.remarks else ""), 
				'debit':self.amount,
				'move_id':journal_entries.id
				})
			create_credit = journal_entries_lines.create({
			...
			# 		y.account_id = self.bank.id
			# 		y.partner_id = self.party.id
			# 		y.name = "Paid" + " "+ str(self.amount) + "  " + "To" + "  "+str(self.party.name) + " in " + str(self.banks_pakistan.name if self.banks_pakistan else " ") + " " + str(self.bank_code if self.banks_pakistan else " ") + " " +"From "+ str(self.bank.code if self.bank else " ")+ " " +str(self.bank.name if self.bank else " ")+ " "+ str(self.remarks if self.remarks else ""),
			# 		y.credit = self.amount
		...
		elif self.type_transaction == "jv":
			party_ledger = self.env['account.account'].search([('name','=',"Party Ledger")])

			create_debit = journal_entries_lines.create({
				'account_id':party_ledger.id,
				'partner_id':self.supplier.id,
				'name':"Payment from" + " "+ str(self.customer.name) + "  " + str(self.banks_pakistan.name if self.banks_pakistan else " ")+ str(self.bank_code if self.banks_pakistan else " ")+" "+"To" + "  "+str(self.supplier.name) + " in " + str(self.supplier_bank.name if self.supplier_bank else " ") + " " + str(self.supplier_bank_code if self.supplier_bank else " ") + " " + str(self.remarks if self.remarks else ""), 
				'debit':self.amount,
				'move_id':journal_entries.id
				})
			create_credit = journal_entries_lines.create({
			...
			# 		y.name = "Payment from" + " "+ str(self.customer.name) + "  " + str(self.banks_pakistan.name if banks_pakistan else " ")+ str(self.bank_code if banks_pakistan else " ")+" "+"To" + "  "+str(self.supplier.name) + " in " + str(self.supplier_bank.name if self.supplier_bank else " ") + " " + str(self.supplier_bank_code if self.supplier_bank else " ") + " " + str(self.remarks if self.remarks else ""), 
			# 		y.credit = self.amount
	...
	def back_to_clearance(self):
		...
		journal_entry = self.env['account.move'].search([('funds_flow_id','=',self.id)])
...
class SummaryEntry(models.Model): 
	...
	payable = fields.Float(digits=(19,0))
	receivable = fields.Float(digits=(19,0))
	net = fields.Float(digits=(19,0))
	balance = fields.Float(compute = "get_ledger_balances",digits=(19,0))
	virtual_balance = fields.Float(string = "Virtual Balance",compute = "get_ledger_balances",digits=(19,0))
	temporary = fields.Float(digits=(19,0))


	@api.one
	def get_ledger_balances(self):
		entries= self.env['account.move.line'].search([('partner_id','=',self.customer.id),('move_id.state','=',"posted"),('account_id.name','in',["Party Ledger"])])
		entries_virtual= self.env['account.move.line'].search([('partner_id','=',self.customer.id),('account_id.name','=',"Party Ledger")])
		debit = 0
		credit = 0
		for x in entries:
			debit = debit + x.debit
...
class SummaryClearance(models.Model): 
	...
	def get_ledger_balances(self):
		entries= self.env['account.move.line'].search([('partner_id','=',self.customer.id),('move_id.state','=',"posted"),('account_id.name','in',["Party Ledger"])])
		entries_virtual= self.env['account.move.line'].search([('partner_id','=',self.customer.id),('account_id.name','in',["Party Ledger"])])
	...
	def sent_for_confirmed(self):
		...
		for y in active_class:
			if y.stages != "clearing":
				...
				journal_entries = self.env['account.move'].search([('funds_flow_id','=',y.id)])
...
class AccountMove(models.Model):
	...
	def get_opening(self):
		opening = self.env['account.journal'].search([('name','=',"Opening")])
		if self.opening == True:
			self.journal_id = opening.id

	# def update_jentry(self):
	# 	print "xxxxxxxxxxxxxxxxxxxxxxxxxx"
		
	# 	opening = self.env['account.move'].search([('journal_id','=',3)])
	# 	total_entries = 0
	# 	for x in opening:
	# 		count = 0
	# 		for y in x.line_ids:
...
class CashBookExtension(models.Model):
	...
	opening = fields.Float(compute = "get_transaction_balance",digits=(19,0))
	transactions = fields.Float(compute = "get_transaction_balance",digits=(19,0))
	funds_flow = fields.Float(string = "Funds Flow",digits=(19,0))
	closing = fields.Float(compute = "get_transaction_balance",digits=(19,0))
	...
	def RaiseValidation(self):
		...
		records = self.env['account.bank.statement'].search([('state','=',"confirm"),('id','!=',self.id)])
		if records:
...
class customer_payments_sugar(models.Model): 
	...
	total_amount = fields.Float(digits=(19,0))
	stages                  = fields.Selection([
		('draft', 'Draft'),
		('posted', 'Posted'),
