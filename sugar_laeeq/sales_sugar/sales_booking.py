	...
	def get_for_sale(self):
		if self.mill:
			...
			if self.sales_booking_id.check == True:
				...
				self.available_sale = for_sale.for_sale_c
			else:
				self.available_sale = for_sale.for_sale_c - for_sale.temporary
...
class sale_sugar(models.Model): 
	...
	def unlink(self):
		...
		amanats = self.env['amanats'].search([('mill','=',mill),('customer','=',customer)])
		amanats.update_values()
		return True
