...
class purchase_booking_treeview(models.Model):
	...
	def create(self, vals):
		...
		create_comm = new_record.create_commission()
	...
	def write(self, vals):
		...
		relevant_commission = self.env['sugar.commission'].search([('purchase_id','=',self.id)])
		if relevant_commission:
			relevant_commission.unlink()
		super(purchase_booking_treeview, self).write(vals)
		after_write = self.qty
		difference =  after_write - before_write

		create_comm = self.create_commission()
	...
	def unlink(self):
		...
		relevant_commission = self.env['sugar.commission'].search([('sale_id','=',self.id)])
		if relevant_commission:
			relevant_commission.unlink()
	...
	def create_commission(self):
		if self.com_type != "no":
			commission = self.env['sugar.commission']
			...
			generate_commission = commission.create({
				...
				'commission_agent':self.commission_agent.id,
				'comm_type':self.com_type,
...
class purchase_sugar(models.Model): 
	...
	def write(self, vals):
		...
		# if not amanats:
		# 	create_amanat = amanats.create({
		# 		'customer': self.supplier.id,
		# 		'mill':self.mill.id,
		# 		})
		# amanats = self.env['amanats'].search([('mill','=',self.mill.id),('customer','=',self.supplier.id)])
		# amanats.write({})
		# print amanats.purchases_c
	...
	def unlink(self):
		...
		amanats = self.env['amanats'].search([('mill','=',mill),('customer','=',supplier)])
		amanats.update_values()
		return True
