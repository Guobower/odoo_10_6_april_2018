...
class StockMovementAddaReport(models.Model):
	...
	openings = fields.Float(string="opening")

	ismill = fields.Boolean(string="Is a mill")

	stock_tree = fields.One2many('stock.movement.tree', "tree_link")

	@api.multi
	def genrate_lines(self):
		active_wizard = self.env['stock.movement.tree'].search([('tree_link','=',self.id)])
		...
		if self.ismill == False:
			loadings = self.env['loading.adda.tree'].search([('date','>=',self.form),('date','<=',self.to),('adda.id','=',self.adda_field.id),('mill.id','=',self.mill.id),('party.id','=',self.party.id)])

		if self.ismill == True:
			loadings = self.env['loading.adda.tree'].search([('date','>=',self.form),('date','<=',self.to),('adda.id','=',self.adda_field.id),('mill.id','=',self.mill.id),('mill_loaded.id','=',self.party.id)])

		open_transfers = self.env['loading.sugar.tree'].search([('date','<',self.form),('adda.id','=',self.adda_field.id),('mill.id','=',self.mill.id),('transfer_type','=','g2c'),('party.id','=',self.party.id)])

		if self.ismill == False:
			open_loadings = self.env['loading.adda.tree'].search([('date','<',self.form),('adda.id','=',self.adda_field.id),('mill.id','=',self.mill.id),('party.id','=',self.party.id)])

		if self.ismill == True:
			open_loadings = self.env['loading.adda.tree'].search([('date','<',self.form),('adda.id','=',self.adda_field.id),('mill.id','=',self.mill.id),('mill_loaded.id','=',self.party.id)])
		...
		trans_open = 0
		for x in open_transfers:
			trans_open = trans_open + x.qty

		load_open = 0
		for x in open_loadings:
			load_open = load_open + x.qty

		self.openings = trans_open - load_open
