...
class EmployeeGatepass(models.AbstractModel):
	...
	def render_html(self,docids, data=None):
		...
		if type_report == 'confirmation':
			print "yes its confirmed"
			entry_level = self.env['funds.flow.tree'].search([('stages','=','confirmation'),('date_confirmation','>=',form),('date_confirmation','<=',to)])
		...
		def opening(party):
			...
			if type_report == "confirmation":
				...
				journal_lines = self.env['account.move.line'].search([('partner_id','=',customers.id),('account_id.name','in',["Party Ledger"]),('move_id.state','=',"posted"),('date','<',form)])
