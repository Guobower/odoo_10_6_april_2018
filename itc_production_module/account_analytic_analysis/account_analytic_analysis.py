...
class account_analytic_invoice_line(models.Model):
    ...
    # price_subtotal = fields.Float(_amount_line, string='Sub Total', type="float",digits_compute= dp.get_precision('Account'))
...
class account_analytic_account(models.Model):
    ...
    is_overdue_quantity = fields.Boolean(method=True, string='Overdue Quantity')
    ca_invoiced  = fields.Float(type='float', string='Invoiced Amount',
        help="Total customer invoiced amount for this account.")
    total_cost  = fields.Float(type='float', string='Total Costs',
    ...
    ca_to_invoice  = fields.Float( multi='analytic_analysis', type='float', string='Uninvoiced Amount',
        help="If invoice from analytic account, the remaining amount you can invoice to the customer based on the total costs.")
    ca_theorical  = fields.Float( multi='analytic_analysis', type='float', string='Theoretical Revenue',
        help="Based on the costs you had on the project, what would have been the revenue if all these costs have been invoiced at the normal sale price provided by the pricelist.")
    hours_quantity  = fields.Float( multi='analytic_analysis', type='float', string='Total Worked Time',
    ...
    last_invoice_date  = fields.Date( multi='analytic_analysis', type='date', string='Last Invoice Date',
        help="If invoice from the costs, this is the date of the latest invoiced.")
    last_worked_invoiced_date  = fields.Date( multi='analytic_analysis', type='date', string='Date of Last Invoiced Cost',
        help="If invoice from the costs, this is the date of the latest work or cost that have been invoiced.")
    last_worked_date  = fields.Date( multi='analytic_analysis', type='date', string='Date of Last Cost/Work',
        help="Date of the latest work done on this account.")
    hours_qtt_non_invoiced  = fields.Float( multi='analytic_analysis', type='float', string='Uninvoiced Time',
    ...
    hours_qtt_invoiced  = fields.Float( type='float', string='Invoiced Time',
        help="Number of time (hours/days) that can be invoiced plus those that already have been invoiced.")
    remaining_hours  = fields.Float( type='float', string='Remaining Time',
        help="Computed using the formula: Maximum Time - Total Worked Time")
    remaining_hours_to_invoice = fields.Float( type='float', string='Remaining Time',
        help="Computed using the formula: Expected on timesheets - Total invoiced on timesheets"),
    fix_price_to_invoice = fields.Float( type='float', string='Remaining Time',
        help="Sum of quotations for this contract.")
    timesheet_ca_invoiced = fields.Float(type='float', string='Remaining Time',
        help="Sum of timesheet lines invoiced for this contract.")
    remaining_ca = fields.Float( type='float', string='Remaining Revenue',
        help="Computed using the formula: Max Invoice Price - Invoiced Amount.")
    revenue_per_hour = fields.Float( type='float', string='Revenue per Time (real)',
        help="Computed using the formula: Invoiced Amount / Total Time")
    real_margin = fields.Float( type='float', string='Real Margin',
        help="Computed using the formula: Invoiced Amount - Total Costs.")
    theorical_margin = fields.Float(type='float', string='Theoretical Margin',
        help="Computed using the formula: Theoretical Revenue - Total Costs")
    real_margin_rate = fields.Float( type='float', string='Real Margin Rate (%)',
    ...
    # month_ids   = fields.Many2many('account_analytic_analysis.summary.month', string='Month')
    # user_ids   = fields.Many2many('account_analytic_analysis.summary.user', string='User')
    hours_qtt_est   = fields.Float('Estimation of Hours to Invoice')
    est_total = fields.Float( type="float",multi="sum_of_all", string="Total Estimation")
    invoiced_total  = fields.Float( type="float",multi="sum_of_all", string="Total Invoiced")
    remaining_total  = fields.Float(type="float",multi="sum_of_all", string="Total Remaining", help="Expectation of remaining income for this contract. Computed as the sum of remaining subtotals which, in turn, are computed as the maximum between '(Estimation - Invoiced)' and 'To Invoice' amounts")
    toinvoice_total  = fields.Float( type="float",multi="sum_of_all", string="Total to Invoice", help=" Sum of everything that could be invoiced for this contract.")
