...
class AccountInvoiceBcube(models.Model):
	...
	dollar_rate = fields.Float(string = "Dollar Rate",digits=(3, 4))
	...
	def amount_untaxed_round(self):
		if self.amount_untaxed:
			untax=self.amount_untaxed
			self.amount_untaxed= round(untax)
				...
				if self.tax_line_ids:
					for line in self.tax_line_ids:
						if line.name == tax.name:
							line.amount = round(line.amount + amount_tax)
...
class AccountInvoiceLineBcube(models.Model):
	...
	assessed = fields.Float(string="Assessed/Unit PKR")
	per_unit_cost = fields.Float(string="Per Unit Cost")
	tax_Amount = fields.Float(string="Tax Amount")
	assessed_dollar = fields.Float(string="Assessed/unit $")
	declared_dollar = fields.Float(string="Declared/Unit $")


	line_taxes = fields.Many2many('account.tax',string="Taxes")

	amount_subtoal = fields.Float(string="Amount",digits=(12))
...
# class AccountInvoiceTaxBcube(models.Model):
