...
class StockMovementAddaReport(models.Model):
	...
	def genrate_lines(self):
		...
		transfers = self.env['loading.sugar.tree'].search([('date','>=',self.form),('date','<=',self.to),('adda.id','=',self.adda_field.id),('mill.id','=',self.mill.id),('transfer_type','=','g2c')])

		loadings = self.env['loading.adda.tree'].search([('adda.id','=',self.adda_field.id),('mill.id','=',self.mill.id)])
