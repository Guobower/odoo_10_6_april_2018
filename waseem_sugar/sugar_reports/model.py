...
class EmployeeGatepass(models.AbstractModel):
	...
	def render_html(self,docids, data=None):
		...
		def opening(party):
			customers = party
			journal_lines= self.env['account.move.line'].search([('partner_id','=',customers.id),('account_id.name','in',["Party Ledger"]),('move_id.state','=',"posted")])
		...
		def get_record_list(attr):
			...
			for x in entry_level:
				...
				if x.party == attr and x.type_transaction == 'br':
					recipt_entries.append(x)
				
				if x.party == attr and x.type_transaction == 'bp':
					payment_entries.append(x)

				if x.customer == attr and x.type_transaction == 'jv':
					journal_entries.append(x)

				if x.supplier == attr and x.type_transaction == 'jv':
					journal_entries.append(x)
